...
def readWordVectorsOld(filename):
...
def readWordVectors(filename):
    ...
    headerlist = fileObject.readline().strip().split()
    vectorDim = int(headerlist[1])
    numVects = int(headerlist[0])
    #vectors = numpy.loadtxt(filename, dtype=float, comments=None, skiprows=1, usecols=range(1, vectorDim + 1))
    vectors = numpy.zeros((numVects, vectorDim))

    wordVectors = {}
    lineNum = 0
    for line in fileObject:
        splitlist = line.lower().strip().split()
        word = splitlist[0]
        vectors[lineNum, :] = numpy.asarray(splitlist[1:])
        wordVectors[word] = vectors[lineNum]
        lineNum += 1
